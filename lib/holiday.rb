...
def all_supplies_in_holidays(holiday_hash)
  holiday_hash.each do |season, holiday|
    ...
    holiday.each do |holiday, item|
      puts "  #{holiday.to_s.split("_").map {|i| i.capitalize}.join(" ")}: #{item.join(", ")}"# iterate through holiday_hash and print items such that your readout resembles:
  # Winter:
  #   Christmas: Lights, Wreath
...
def all_holidays_with_bbq(holiday_hash)
  holiday_hash.each do |holidays, supplies|
    if supplies === "BBQ"
      puts holiday_hash# return an array of holiday names (as symbols) where supply lists
